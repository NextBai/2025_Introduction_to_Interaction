...
//想要把聲音最需要的頻率(Hz)找出來,使用 PitchDector 物件

import processing.sound.*;
SoundFile sound;//新加這一行,因為要把beat.mp3讀入
PitchDetector pitchDetector;//宣告PitchDector物件變數
int i;
...
void setup() {
  ...
  sound.loop();
  pitchDetector = new PitchDetector(this);//新增, 初始化 PitchDector物件
  // capture audio input
  // pitchDetector.input(new AudioIn(this, 0));//原本程式是用Audio In音效卡的音源輸入
